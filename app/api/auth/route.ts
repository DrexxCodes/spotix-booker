 ...
 * Auth API Route
 * 
 * POST /api/auth - Create session with ID token and verify booker status
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Fetch user document from Firestore to get isBooker status
    let isBooker = false;
    let userData = null;
    let balance = 0;
    ...
    // Set isBooker cookie
    cookieStore.set("isBooker", isBooker.toString(), {
...
export async function GET(request: NextRequest) {
  try {
    ...
    const isBooker = cookieStore.get("isBooker")?.value === "true";
