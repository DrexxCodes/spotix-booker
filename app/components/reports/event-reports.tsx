...
import { AlertCircle, CheckCircle, Clock, ChevronRight, ArrowLeft, Search, Filter, Calendar } from "lucide-react"
...
interface EventReportsComponentProps {
  userId: string
}

export function EventReportsComponent({ userId }: EventReportsComponentProps) {
  ...
  useEffect(() => {
    const fetchEventIds = async () => {
      try {
        const eventsCollectionRef = collection(db, "reports", userId, "events")
  ...
  useEffect(() => {
    ...
    const fetchReports = async () => {
      setLoading(true)
      try {
        const reportsCollectionRef = collection(db, "reports", userId, "events", selectedEventId, "reports")
  ...
  if (eventIds.length === 0) {
    return (
      <div className="text-center py-16 bg-gradient-to-br from-slate-50 to-purple-50/30 rounded-2xl border-2 border-dashed border-slate-200">
        ...
        <p className="text-slate-600">There are no event reports at the moment.</p>
      </div>
    )
  }

  // Show list of event IDs
  if (!selectedEventId) {
    return (
      <div className="space-y-6 animate-in fade-in duration-700">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h2 className="text-3xl font-bold text-slate-900">Select an Event</h2>
        </div>
          ...
          {eventIds.map((eventId) => (
            ...
            >
              <div className="flex-1 text-left">
                ...
                <p className="text-sm text-slate-500 font-mono truncate">{eventId}</p>
  ...
  // Show reports for selected event
  return (
    <div className="space-y-6 animate-in fade-in duration-700">
      {/* Header */}
      <div className="flex flex-col gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-3xl font-bold text-slate-900">Event Reports</h2>
            <p className="text-sm text-slate-600 font-mono mt-1">ID: {selectedEventId}</p>
          </div>
        </div>
      </div>
