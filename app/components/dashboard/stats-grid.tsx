  ...
  >
    ...
    <div className="flex items-start gap-4 relative z-10">
      <div className={`p-3 rounded-lg transition-all duration-300 group-hover:scale-110 group-hover:rotate-6 ${
        ...
        <Icon size={24} className={highlight ? "text-white animate-[bounce_1s_ease-in-out_infinite]" : "text-[#6b2fa5] group-hover:text-[#8b4fc5]"} />
      </div>
      <div className="flex-1">
        <p className={`text-sm font-medium transition-colors duration-300 ${
          highlight 
            ? "text-white/90" 
            : "text-gray-600 group-hover:text-[#6b2fa5]"
        }`}>
          {label}
        </p>
...
export function StatsGrid({ stats }: StatsGridProps) {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
      ...
      <StatCard icon={Calendar} label="Past Events" value={formatNumber(stats.pastEvents)} />
